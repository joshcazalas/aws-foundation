#!/usr/bin/env python3
"""Render sanitized OpenTofu plan results as one bounded PR comment."""
...
MARKER = "<!-- aws-foundation-terraform-plan -->"
MAX_COMMENT_LENGTH = 60_000
TRUNCATION_MESSAGE = "Plan detail truncated. Use the CI-run link for the complete log."

ROOTS = (
    ("terraform/bootstrap/management-state", "management-state", ("management",)),
    ("terraform/organization", "organization", ("management",)),
    ("terraform/platform", "platform", ("production", "uat", "deployment")),
    ("terraform/github", "github", ("github",)),
)
SCOPES = (
...
def scope_status(scope: str, roots: list[RootResult]) -> str:
    return "failed" if any(scope in root.scopes and root.status != "success" for root in roots) else "success"
...
def truncate_body(body: str, limit: int) -> str:
    ...
    fence_index = next(
        (index for index, line in enumerate(lines) if line.endswith("diff") and set(line[:-4]) == {"`"}),
        None,
    )
...
def render_comment(results_directory: Path, repository: str, run_id: str) -> str:
    ...
    for scope, label in SCOPES:
        ...
        table_rows.append(
            f"| {label} | {summary(counts.add, counts.change, counts.destroy, scope_status(scope, roots))} |"
        )
    ...
    # Reserve every heading, details summary, closing tag, and a clear omission
    # notice first. Spend the remaining budget on plan bodies in production-first
    # order. This guarantees every root result survives deterministic truncation.
    ...
    for section_header, details in sections:
        ...
        fixed_length += sum(len(opening) + len(minimal_body) + len(closing) + 2 for opening, _, closing in details)
